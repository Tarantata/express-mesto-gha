...
const updateUserProfile = async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(owner, { name, about }, { new: true, runValidators: true });
    if (user !== user) {
      return res.status(400).json({message: 'Введены некорректные данные при создании пользователя'});
    }
    if (user === null) {
...
const updateUserAvatar = async (req, res) => {
  ...
  const {avatar} = req.body;
    const owner = req.user._id;
    const user = await User.findByIdAndUpdate(owner, { avatar }, { new: true, runValidators: true });
